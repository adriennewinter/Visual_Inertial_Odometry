...
with rosbag.Bag(output_bag_name, 'w') as outbag:
    ...
    from sensor_msgs import Image
    img0_sub = message_filters.Subscriber('/video_source_0/raw', Image)
    img1_sub = message_filters.Subscriber('/video_source_1/raw', Image)

    # ApproximateTimeSynchronizer wants a list of subscribers
    subscriber_list = [img0_sub, img1_sub]
    ...
    def callback(*msgs):
        ...
        for topic_name, msg in itertools.izip(topic_names, msgs):
            ...
            # This is because 1) We need to choose a timestamp from one of the input messages
